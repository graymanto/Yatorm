...
namespace Yatorm
{
    ...
    {
        private static readonly Func<Type, Delegate> _paramMapperFunc = new Func<Type, Delegate>(
            MakeParamMapper
        ).Memoize();

        private static readonly ConcurrentDictionary<string, object> _cachedMappers = new();
        ...
        /// <returns>
        /// </returns>
        public static IQueryable<T> GetMappedSequence<T>(this IDataReader rdr)
            where T : new()
        {
        ...
        public static IEnumerable<SqlParameter>? TransformClassToSqlParameters<T>(
            T input,
        ...
        private static Func<IDataReader, TMapped> MakeReaderToClassMapper<TMapped>(IEnumerable<string> cols)
        {
            ...
            )
            {
                ...
                var ifNotNullExpression = Expression.IfThen(
        ...
        private static Delegate? MakeParamMapper(Type paramType, IEnumerable<string>? excludeColumns)
        {
            ...
            var expr = Expression.Lambda(Expression.Block(variables, body.ToArray()), source);
